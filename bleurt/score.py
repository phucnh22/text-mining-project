  ...
  def predict(self, input_dict):
    predictions = self._bleurt_model_ops(
        ...
        segment_ids=tf.constant(
            input_dict["segment_ids"],dtype = tf.int64))["predictions"].numpy()
    return predictions
    ...
    for i in range(0, len(candidates), batch_size):
      ...
      if self.serialize_input:
        ...
        predict_out = self.bleurt_model_ops(
            examples=tf.constant(tfrecords,dtype = tf.int64))["predictions"].numpy()
      else:
        predict_out = self.bleurt_model_ops(
            references=tf.constant(batch_ref,dtype = tf.int64),
            candidates=tf.constant(batch_cand,dtype = tf.int64))["predictions"].numpy()
